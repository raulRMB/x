...
void MainScene::Start()
{
    ...
    x::Renderer::Get().CreateSkeletalMesh("assets/models/Lucha.fbx");
...
void MainScene::Update(f32 deltaTime)
{
    ...
    for(entt::entity entity : view)
    {
        ...
        SkeletalMesh::GetPose(skeletalMesh.GetAnimation(), skeletalMesh.GetRootBone(), deltaTime, skeletalMesh.GetCurrentPose(), skeletalMesh.GetRootBone().OffsetMatrix, skeletalMesh.GetGlobalInverseTransform());
    }
}
...
void MainScene::HandleInput(const SDL_Event &event)
{
    ...
    else if(event.type == SDL_KEYDOWN)
    {
        if(event.key.keysym.sym == SDLK_s)
        {
            ...
            AddComponent(e, transform);
            AddComponent(e, CTriangleMesh(x::Renderer::Get().CreateMeshModel("assets/models/ball.fbx")));
