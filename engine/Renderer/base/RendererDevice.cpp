...
#include <string.h>
    ...
    QueueFamilyIndices RendererDevice::GetQueueFamilies(VkPhysicalDevice device) {
    ...
    SurfaceDetails RendererDevice::GetSurfaceDetails(VkPhysicalDevice device)
    {
    ...
    void RendererDevice::CreateLogicalDevice() {
        ...
        #ifdef __APPLE__
        const char* const* currentExtensions = DeviceExtensions.data();
        uint32_t currentExtensionCount = static_cast<uint32_t>(DeviceExtensions.size());
