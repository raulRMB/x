...
i32 Renderer::Init()
{
    try
    {
        ...
        QueueFamilyIndices indices = RenderDevice->GetQueueFamilies(MainDevice.PhysicalDevice);
...
void Renderer::CreateSkeletalPipeline()
{
    ...
    attributeDescriptions[1].offset = offsetof(SkeletalVertex, Col);

    attributeDescriptions[2].binding = 0;
    attributeDescriptions[2].location = 2;
    attributeDescriptions[2].format = VK_FORMAT_R32G32_SFLOAT;
    attributeDescriptions[2].offset = offsetof(SkeletalVertex, Tex);
    ...
    attributeDescriptions[4].offset = offsetof(SkeletalVertex, BoneWeights);
...
{
    QueueFamilyIndices queueFamilyIndices = RenderDevice->GetQueueFamilies(MainDevice.PhysicalDevice);
...
void Renderer::CreateUniformBuffers()
{
    ...
    for(size_t i = 0; i < swapchain->SwapchainImages.size(); i++)
    {
        ...
        CreateBuffer(MainDevice.PhysicalDevice, MainDevice.LogicalDevice, boneBufferSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                                    VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
...
{
    DepthBufferImage = CreateImage(swapchain->SwapchainExtent.width, swapchain->SwapchainExtent.height,
    ...
    DepthBufferImageView = CreateImageView(DepthBufferImage, DepthBufferImageFormat, VK_IMAGE_ASPECT_DEPTH_BIT, MainDevice.LogicalDevice);
}
...
i32 Renderer::CreateTextureImage(const std::string& fileName)
{
    ...
    TransitionImageLayout(MainDevice.LogicalDevice, MainDevice.GraphicsQueue, GraphicsCommandPool, texImage, VK_IMAGE_LAYOUT_UNDEFINED,
                          VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);

    CopyImageBuffer(MainDevice.LogicalDevice, MainDevice.GraphicsQueue, GraphicsCommandPool,
                    imageStagingBuffer, texImage, width, height);

    TransitionImageLayout(MainDevice.LogicalDevice, MainDevice.GraphicsQueue, GraphicsCommandPool, texImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
...
i32 Renderer::CreateTexture(const std::string &fileName)
{
    ...
    VkImageView imageView = CreateImageView(TextureImages[textureImageLoc],
...
void Renderer::CreateMesh(const std::string& texture, Shape shape, const glm::vec4& color)
{
    ...
    {
        case Shape::Square:
            Verts = MakeSquare(color);
            Indices = { 0, 1, 2, 3, 0 };
            break;
        case Shape::Triangle:
            break;
        case Shape::Circle:
            Verts = MakeCircle(color, 20);
