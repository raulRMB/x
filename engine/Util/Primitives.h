...
#include "../Renderer/RendererUtil.h"
    ...
    inline std::vector<struct Vertex> MakeSquare(const glm::vec4& color)
    {
    ...
    inline std::vector<Vertex> MakeFilledCircle(const glm::vec4& color, u32 segments)
    {
        std::vector<Vertex> vertices;
    ...
    inline std::vector<Vertex> MakeCircle(const glm::vec4& color, u32 segments)
    {
        std::vector<Vertex> vertices;
        vertices.reserve(segments + 1);
        float angle = 0.0f;
...
typedef Square Square2D;
typedef Circle Circle2D;
typedef Triangle Triangle2D;
typedef Edge Edge2D;
